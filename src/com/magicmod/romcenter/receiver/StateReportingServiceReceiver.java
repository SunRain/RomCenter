...
package com.magicmod.romcenter.receiver;
...
import com.magicmod.romcenter.service.StateReportingService;
import com.magicmod.romcenter.utils.Constants;

public class StateReportingServiceReceiver extends BroadcastReceiver {
    private static final String TAG = "ReportingServiceReceiver";
    ...
    public static void setAlarm(Context context, long millisFromNow) {
        SharedPreferences prefs = RomStateFragment.getPreferences(context);
        boolean optedIn = prefs.getBoolean(Constants.ANONYMOUS_OPT_IN, true);
        if (!optedIn) {
            return;
        }

        if (millisFromNow <= 0) {
            long lastSynced = prefs.getLong(Constants.ANONYMOUS_LAST_CHECKED, 0);
            if (lastSynced == 0) {
                ...
                prefs.edit().putLong(Constants.ANONYMOUS_LAST_CHECKED, lastSynced).apply();
                Log.d(TAG, "Set alarm for first sync.");
            }
            millisFromNow = (lastSynced + UPDATE_INTERVAL) - System.currentTimeMillis();
        }

        Intent intent = new Intent(ConnectivityManager.CONNECTIVITY_ACTION);
        intent.setClass(context, StateReportingServiceReceiver.class);
    ...
    public static void launchService(Context context) {
        ...
        intent.setClass(context, StateReportingService.class);
